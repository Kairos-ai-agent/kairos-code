...
3. **Cost cap (wall-clock)** — `time.time() - session.started_at ≥ COST_TIME_CAP_S` (default 30 min)
...
- **LoopReview engine** — Coder ↔ Reviewer auto-loop with 6 gates
...
│   ├── loop/
│   │   └── review_loop.py     # run_loop + 6 gates + plan-mode gate
│   ├── review/
