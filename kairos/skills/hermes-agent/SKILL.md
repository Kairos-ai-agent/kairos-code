...
source-path: "C:\\Users\\leohu\\AppData\\Local\\hermes\\skills\\autonomous-ai-agents\\hermes-agent\\SKILL.md"
---
# Hermes Agent
...
**`hermes gateway start` fails on Chinese Windows** — the Scheduled Task subprocess output contains GBK-encoded Chinese text, causing `UnicodeDecodeError` in Python's UTF-8 decoder. Gateway may actually start but the launcher reports "no process detected." Workaround: run `hermes gateway run` in foreground (bypasses Scheduled Task). Full recipe + Weixin session expiry and DNS cache issues: `references/windows-gateway-troubleshooting.md`.

**Diagnosing "messaging platform can't connect"** — when a user reports a platform like Weixin/Telegram/Discord can't connect, check `hermes status` for gateway status FIRST before network diagnostics. The most common cause is the gateway process not running, not a network issue.

### GatewayManager / Web UI

**`process.kill(pid, 0)` doesn't work on Windows** — `GatewayManager.detectStatus()` in `hermes-web-ui` uses `process.kill(pid, 0)` (a POSIX-only signal-0 probe) to check if a gateway process is alive. On Windows this throws EINVAL and returns false, causing the GatewayManager to:
  1. Fail to detect a running gateway on the configured port
  2. Reassign the port (e.g. 8642 → 8643 → 8644)
  3. Try to start a new gateway process, which hangs/times out
  4. Block the entire server startup (Web UI exits with "timeout response")

**Fix (patch `hermes-web-ui/dist/server/services/hermes/gateway-manager.js`):**
- `detectStatus()`: add a health-only fallback after the PID check — just `await this.checkHealth(url)` directly, without requiring PID
- `startAll()`: replace `this.isProcessAlive(existing.pid)` with `await this.checkHealth(existing.url, 2000)` in Phase 1's "already running" check
- Also fix the "stale PID" check in `startAll()`: use `!(await this.checkHealth(cfgUrl, 2000))` instead of `this.isProcessAlive(pid)`

**Desktop shortcut / launcher script:** Don't use `hermes-web-ui status` (which also calls `process.kill(pid, 0)`) to check if the server is already running. Use `curl http://localhost:8648/health` instead.

**Full startup sequence for a reliable Windows setup:**
1. Start gateway manually: `hermes gateway run` (in a separate terminal, or let the Desktop shortcut's curl check bypass if already running)
2. Create `~/.hermes/gateway.pid` with `{"pid": <actual_pid>}` so other tools can find it
3. Start Web UI: `npx hermes-web-ui` or use the bat file (which now uses health-endpoint checks)
4. Open http://localhost:8648 with the token from the server log
