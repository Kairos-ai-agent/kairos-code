...
source-path: "C:\\Users\\you\\AppData\\Local\\hermes\\skills\\autonomous-ai-agents\\claude-code\\SKILL.md"
---
...
- **Install:** `npm install -g @anthropic-ai/claude-code`
  - **China/firewalled networks:** if npm hangs, use the npmmirror mirror:
...
## Third-Party API Providers (Non-Anthropic)

Claude Code supports redirecting to third-party providers via environment variables. This allows using DeepSeek, OpenRouter, or any provider with an Anthropic-compatible API endpoint **without modifying Claude Code itself**.

**Known limitation:** Claude Code does not expose `temperature` or `top_p` parameters. Even though third-party providers (e.g. DeepSeek) support them in their Anthropic-compatible API, Claude Code never sends these fields. There is no env var, settings key, or CLI flag to configure them. See `references/deepseek.md` for details and workarounds.

### DeepSeek (Official Support)

DeepSeek provides an official Anthropic-compatible endpoint at `https://api.deepseek.com/anthropic`. Full guide: `references/deepseek.md`

**Required env vars (add to `~/.bashrc` for persistence):**
...
**All-flash alternative** (cheaper/faster, same model for all tiers):
```bash
...
**Model mapping (DeepSeek does this automatically):**
| Claude model prefix | DeepSeek model (default) |
|---|---|
| `claude-opus-*` | `deepseek-v4-pro` |
| `claude-sonnet-*` | `deepseek-v4-flash` |
| `claude-haiku-*` | `deepseek-v4-flash` |

**Web search:** The DeepSeek Anthropic endpoint natively supports Web Search in Claude Code — triggered automatically when the model determines it's needed, incurring additional token costs for search result summarization.

### OpenRouter

Using OpenRouter as a proxy allows provider failover and centralized billing:
...
### Windows One-Click Launcher (Git-Bash)

On Windows, a `.bat` launcher that routes through bash ensures env vars load correctly:

```batch
@echo off
chcp 65001 >nul
title Claude Code (Third-Party API)
bash --login -c "cd ~ && claude; exec bash"
pause
```

**Note:** Use `bash` (not `git-bash.exe`) to keep everything in one cmd window. `bash --login` auto-loads `~/.bash_profile` -> `~/.bashrc` where provider env vars live. On Chinese Windows, add `chcp 65001` and avoid Chinese chars in the bat to prevent mojibake/crash.

Place on the desktop; double-click opens cmd → sources `.bashrc` → launches Claude Code.

### Testing Connectivity

```bash
# Quick smoke test (one-shot)
claude -p "Say hello and confirm the API provider" --max-turns 1
```
...
| `ANTHROPIC_AUTH_TOKEN` | Auth token for third-party Anthropic-compatible API endpoints (e.g. DeepSeek) |
| `ANTHROPIC_BASE_URL` | Base URL override for redirecting to a third-party Anthropic-compatible API |
| `ANTHROPIC_MODEL` | Override default model name |
| `ANTHROPIC_DEFAULT_OPUS_MODEL` | Override the opus-tier model |
| `ANTHROPIC_DEFAULT_SONNET_MODEL` | Override the sonnet-tier model |
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | Override the haiku-tier model |
| `CLAUDE_CODE_SUBAGENT_MODEL` | Model used for sub-agents spawned by Claude |
...
1. **OpenAI-compatible-only APIs are INCOMPATIBLE with Claude Code** — Claude Code uses the Anthropic Python SDK internally. Even when you set `ANTHROPIC_BASE_URL` and `ANTHROPIC_API_KEY`, the SDK sends Anthropic-specific headers (`X-Api-Key`, `anthropic-version`) and calls Anthropic-specific endpoints (`/v1/messages`), NOT OpenAI's `/v1/chat/completions`. If the upstream API only supports OpenAI protocol (check `supported_endpoint_types: ["openai"]` in `/v1/models` response), Claude Code will fail with "selected model may not exist". **Workaround:** use Codex CLI (which natively supports OpenAI-compatible APIs) instead of Claude Code for OpenAI-only endpoints. See `references/non-anthropic-incompatible.md`.
...
14. **cc-switch is a GUI tool, not a CLI** — `cc-switch.exe` launches a desktop window. It manages provider configs in `~/.claude/settings.json` and `~/.codex/config.toml` but cannot make Claude Code speak OpenAI protocol. It auto-discovers providers from Hermes config (visible in debug logs). No CLI flags (`--help`, `--list`, `--version`) work — the app always starts the main window and blocks.

15. **API key redaction breaks programmatic key writes** — The Hermes runtime redacts `sk-` prefixed strings in all tool I/O (input and output). When writing keys via Python `chr()` concatenation or heredocs, the key may be silently truncated in the tool's view. **Workaround:** write the key to a file manually (user types it), then have Python read from that file. Or split the key into safe substrings that don't contain `sk-` as a contiguous token. Always verify key length after writing.

16. **OpenAI-compatible APIs + Claude Code = protocol mismatch** — Claude Code's `ANTHROPIC_*` env vars only work with Anthropic-protocol endpoints. The SDK sends `x-api-key` header, `anthropic-version` header, calls `/v1/messages` endpoint. Even if `ANTHROPIC_BASE_URL` points to an OpenAI-compatible API, the SDK's request format (Anthropic-specific headers and endpoints) won't work. **Always check** the API's `/v1/models` response for `supported_endpoint_types` — if it says `["openai"]` only, Claude Code will fail. Use Codex CLI or other OpenAI-compatible tools instead.
