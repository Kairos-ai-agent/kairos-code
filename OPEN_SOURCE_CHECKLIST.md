...
| **Hardcoded machine paths** | Fixed and now guarded. The one real offender was a Windows-only build script that pinned a maintainer drive and could not run for anyone else: it is **deleted**, replaced by `scripts/build_binary.py`, which derives the repo root from `__file__` and builds on Windows/macOS/Linux (plus `scripts/smoke_binary.py`, which starts the frozen binary and asserts it serves the UI). Elsewhere: `scripts/gen_languages.mjs` derives the root from `import.meta.url`; `kairos/bench/real_eval.py` reads environment variables; `web/public/branding/_build_icons.py` takes argv; docs and the Windows launchers use `<repo>` placeholders. `tests/test_repo_hygiene.py` fails the suite if any tracked file reintroduces a machine path, or if a broad `.gitignore` rule hides a file the build needs (that defect had kept 298 i18n keys out of every clone). |
